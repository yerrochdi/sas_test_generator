...
Pour CHAQUE bloc du workflow :
- Un fichier `bloc_[N]_[nom]_test_data.py` contenant les DataFrames de test
- Un fichier `bloc_[N]_[nom]_branches.md` documentant les branches et la couverture

Plus un fichier de synthèse `SYNTHESE_COUVERTURE.md` récapitulant la
couverture globale de tout le workflow.
...
───────────────────────────────────────────────
ÉTAPE D — Générer le code Python pour ce bloc
───────────────────────────────────────────────

Génère DEUX fichiers pour ce bloc :

FICHIER 1 : ./test-plan/data/bloc_[N]_[nom]_test_data.py

   ```python
   """
   Données de test — Bloc [N] : [nom descriptif]
   Programmes couverts : [liste]
   Couverture visée : [SEUIL]%
   """
   import pandas as pd
   import numpy as np
   ...
   def get_input_[dataset]() -> pd.DataFrame:
       """
       [NOM] — [N] observations.
       Source : [externe / sortie du bloc précédent]
       """
       df = pd.DataFrame({
           "col": [
               "val",      # Ligne 1 : BR_xxx VRAI (raison)
               "val",      # Ligne 2 : BR_xxx FAUX (raison)
           ],
       })
       df["col"] = df["col"].astype("string")   # typage strict
       return df


   # ════════════════════════════════════════════
   # SORTIES ATTENDUES DU BLOC
   # ════════════════════════════════════════════

   def get_expected_[dataset_sortie]() -> pd.DataFrame:
       """
       Résultat attendu.
       Source : estimé par analyse du code SAS.
       """
       ...
   ...
   QUALITÉ OBLIGATOIRE :
   - CHAQUE valeur a un commentaire avec la branche activée
   - Types explicites : float64 (num SAS), "string" (char SAS), datetime64[ns] (date SAS)
   - DataFrame JAMAIS vide
   - Pas de colonnes inutiles

   CHAÎNAGE ENTRE BLOCS :
   Si ce n'est pas le premier bloc, les fonctions get_input_*() de ce bloc
   doivent importer les get_expected_*() du bloc précédent :

   ```python
   def get_input_atntmp_tables() -> pd.DataFrame:
       """Entrée = sortie du Bloc [N-1]."""
       from .bloc_[N-1]_[nom]_test_data import get_expected_atntmp_tables
       return get_expected_atntmp_tables()
   ```

FICHIER 2 : ./test-plan/data/bloc_[N]_[nom]_branches.md

   - Liste des branches (BR_xxx) avec leur ID
   - Tableau des données ↔ branches couvertes
   - Taux de couverture atteint pour ce bloc
   - Branches non couvertes et justification
   - Pièges de migration identifiés

───────────────────────────────────────────────
ÉTAPE E — Vérification avant de passer au bloc suivant
───────────────────────────────────────────────
...
□ Le code Python correspond exactement au tableau de données
□ Les commentaires par valeur sont présents et corrects
□ Les types sont explicitement définis

Si une vérification échoue, CORRIGE avant de continuer.

Sauvegarde les fichiers du bloc, puis PASSE AU BLOC SUIVANT.
...
4. PROMPT 1 (un seul lancement)
   → Claude itère automatiquement sur tous les blocs
   → Pour chaque bloc : lit le code → liste les branches → génère les données
   → Chaîne les sorties d'un bloc comme entrées du suivant
   → Vous obtenez tous les [bloc]_test_data.py + SYNTHESE_COUVERTURE.md

   → VÉRIFICATION RAPIDE (2 minutes) :
     • Les DataFrames sont-ils non vides ?
     • La synthèse montre-t-elle une couverture proche du seuil ?
     • Les clés de jointure sont-elles cohérentes entre blocs ?
     Si problème → relancez le prompt 1 en signalant le souci.
